...
- (id)initInScrollView:(UIScrollView *)scrollView activityIndicatorView:(UIView *)activity
{
    ...
    if (self) {
        ...
        self.originalContentInset = UIEdgeInsetsMake(scrollView.contentInset.top + kMaxLabelHeight, scrollView.contentInset.left, scrollView.contentInset.bottom, scrollView.contentInset.right);
        ...
        CGRect frameLabel = CGRectMake(0, self.frame.size.height - kMaxLabelHeight, self.frame.size.width, kMaxLabelHeight);
        _textTitle = [[UILabel alloc] initWithFrame:frameLabel];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if (_refreshing) {
        if (offset != 0) {
            ...
            _textTitle.text = @"Refreshing.";
            [CATransaction begin];
            [CATransaction setValue:(id)kCFBooleanTrue forKey:kCATransactionDisableActions];
            _shapeLayer.position = CGPointMake(0, kMaxDistance + offset + kOpenedViewHeight);
            [CATransaction commit];

            _activity.center = CGPointMake(floor(self.frame.size.width / 2), MIN(offset + self.frame.size.height + floor(kOpenedViewHeight / 2), self.frame.size.height - 30)); //kOpenedViewHeight/ 2));
    ...
    CGPoint bottomOrigin = CGPointMake(floor(self.bounds.size.width / 2), self.bounds.size.height - currentBottomPadding -currentBottomRadius - kMaxLabelHeight);
    CGPoint topOrigin = CGPointZero;
    if (distance == 0) {
        topOrigin = CGPointMake(floor(self.bounds.size.width / 2), bottomOrigin.y);
    } else {
        topOrigin = CGPointMake(floor(self.bounds.size.width / 2), self.bounds.size.height + offset + currentTopPadding + currentTopRadius - kMaxLabelHeight);
        if (percentage == 0) {
...
- (void)endRefreshing
{
    if (_refreshing) {
        ...
        UIEdgeInsets inset = UIEdgeInsetsMake(self.originalContentInset.top - kMaxLabelHeight, self.originalContentInset.left, self.originalContentInset.bottom, self.originalContentInset.right);

        __block UIScrollView *blockScrollView = self.scrollView;
        [UIView animateWithDuration:0.4 animations:^{
            _ignoreInset = YES;
            [blockScrollView setContentInset:inset];
            _ignoreInset = NO;
            _activity.alpha = 0;
            _activity.layer.transform = CATransform3DMakeScale(0.1, 0.1, 1);
